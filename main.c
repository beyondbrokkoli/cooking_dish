...
void build_camera_matrix(float width, float height, float cam_z, float* out_matrix) {
    ...
    float zNear = 100.0f; // Can be closer now!
    ...
    proj[0]  = f / aspect;
    proj[5]  = -f;
    // REVERSE-Z: 1.0 at near plane, 0.0 at infinity
    proj[10] = 0.0f; 
    proj[11] = -1.0f;
    proj[14] = zNear;
...
typedef struct {
    float x, y, z;
    float padding; // The padding float
} VertexAoS;
...
uint32_t findSmartBufferMemory(VkPhysicalDevice physicalDevice, uint32_t typeFilter) {
    VkPhysicalDeviceMemoryProperties memProperties; 
    vkGetPhysicalDeviceMemoryProperties(physicalDevice, &memProperties);

    // ATTEMPT 1: The God-Tier ReBAR Memory (Fast CPU Write + Fast GPU Read)
    VkMemoryPropertyFlags rebarFlags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | 
                                       VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | 
                                       VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;
                                       
    for (uint32_t i = 0; i < memProperties.memoryTypeCount; i++) {
        if ((typeFilter & (1 << i)) && (memProperties.memoryTypes[i].propertyFlags & rebarFlags) == rebarFlags) {
            printf("[VULKAN] ReBAR Supported! Streaming directly to VRAM.\n");
            return i;
        }
    }

    // ATTEMPT 2: Standard PCIe Staging Memory (Fallback)
    VkMemoryPropertyFlags stdFlags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | 
                                     VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
                                     
    for (uint32_t i = 0; i < memProperties.memoryTypeCount; i++) {
        if ((typeFilter & (1 << i)) && (memProperties.memoryTypes[i].propertyFlags & stdFlags) == stdFlags) {
            printf("[VULKAN] ReBAR NOT found. Falling back to System RAM.\n");
            return i;
        }
    }
    
    printf("FATAL: Failed to find suitable buffer memory!\n");
    exit(-1);
}
...
int main() {
    ...
    // Let's allocate enough space for 2.5 Million Tetrahedrons!
    uint32_t maxVerts = 10000000;    // 2.5m * 4
    uint32_t maxIndices = 30000000;  // 2.5m * 12
    ...
    allocV.memoryTypeIndex = findSmartBufferMemory(chosenGPU, reqsV.memoryTypeBits);
    ...
    allocI.memoryTypeIndex = findSmartBufferMemory(chosenGPU, reqsI.memoryTypeBits);
    ...
    attrDesc.binding = 0;
    attrDesc.location = 0;
    // CRITICAL CHANGE: Update from VK_FORMAT_R32G32B32_SFLOAT to a 4-component vector
    ...
    // CRITICAL CHANGE: Turn on hardware backface culling!
    rasterizer.cullMode    = VK_CULL_MODE_BACK_BIT;
    // Tell Vulkan which way your triangles are wound.
    // If your tetrahedrons suddenly disappear, change this to VK_FRONT_FACE_CLOCKWISE
    ...
    depthStencil.depthCompareOp = VK_COMPARE_OP_GREATER; // Closer pixels have GREATER depth values
    ...
    // Place these ABOVE the while loop
    double lastTime = glfwGetTime();
    int frameCount = 0;
    double benchmarkTimer = 0;
    while (!glfwWindowShouldClose(g_window)) {
        glfwPollEvents();
        if (glfwGetKey(g_window, GLFW_KEY_ESCAPE) == GLFW_PRESS) { glfwSetWindowShouldClose(g_window, GLFW_TRUE); }

        // Place this INSIDE the while (!glfwWindowShouldClose(g_window)) loop
        ...
        if (benchmarkTimer >= 1.0) { // Every 1 second
            char title[256];
            double avgMs = (benchmarkTimer / frameCount) * 1000.0;
            sprintf(title, "VibeEngine | FPS: %d | Avg: %.2f ms ", 
                    frameCount, avgMs);
            // glfwSetWindowTitle(g_window, title);
            // 3. Print to Terminal (Standard Output)
            // Adding \r and fflush(stdout) makes it update on a single line!
            printf("\r%s", title);
            fflush(stdout);
            frameCount = 0;
            benchmarkTimer = 0;
        ...
        if (draw_count > 0) {
            // MUST be vkCmdDraw (NOT vkCmdDrawIndexed)
            // Args: (buffer, vertices_per_particle, total_particles, first_vertex, first_instance)
